...
def test_scalar_job():
    from tests.integration_testing.pipelines import scalar_pipeline
    zip_name = scalar_pipeline.__name__.replace('.', '_') + '.tar.gz'
    kfp.compiler.Compiler().compile(scalar_pipeline.pipeline, zip_name)
    submit_and_validate(zip_name)
